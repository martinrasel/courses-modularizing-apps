package de.bembelnaut.courses.modularizingapps.hero_datasource.cache

import de.bembelnaut.courses.modularizingapps.hero_datasource.cache.model.toHero
import de.bembelnaut.courses.modularizingapps.hero_domain.Hero
import de.bembelnaut.courses.modularizingapps.hero_domain.HeroRole
import de.bembelnaut.courses.modularizingapps.hero_interactors.cache.HeroDatabase
import de.bembelnaut.courses.modularizingapps.herointeractors.cache.HeroDbQueries
...
class HeroCacheImpl(
    ...
    override suspend fun getHero(id: Int) =
        queries.getHero(id.toLong()).executeAsOne().toHero()
    ...
    override suspend fun selectAll(): List<Hero> =
        queries.selectAll().executeAsList().map { it.toHero() }
    ...
    override suspend fun searchByName(localizedName: String) =
        queries.searchHeroByName(localizedName).executeAsList().map { it.toHero() }

    override suspend fun searchByAttr(primaryAttr: String) =
        queries.searchHeroByAttr(primaryAttr).executeAsList().map { it.toHero() }

    override suspend fun searchByAttackType(attackType: String) =
        queries.searchHeroByAttackType(attackType).executeAsList().map { it.toHero() }
